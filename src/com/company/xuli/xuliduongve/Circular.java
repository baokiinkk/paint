...
public class Circular extends HinhHoc {
    ...
    public void setCircular(Point2D start, Point2D end, lineMode mode) {
        ...
        centerElipT.set(Math.abs(start.X - end.X)/2 + start.X,start.Y);
        centerElipD.set(Math.abs(start.X - end.X)/2 + start.X,end.Y);
        Major_rad = Math.abs(centerElipT.X - this.end.X);
        Minor_rad = Major_rad/3;
    }
    public void drawElip(Point2D point) {
        int x1_c = 0;
    ...
    public void draw() {
        drawElip(centerElipT);
        drawElip(centerElipD);
